...
\begin{tikzpicture}[x=1pt, y=1pt, outer sep=0pt,
  ...
  \node[module=attnblue, above=27pt of dec] (pwb) {Pending-Load Writeback};
  \node[module=attnblue, above=27pt of pwb] (opf) {Operand Fetch};
  \node[halfmod=moegreen, above=27pt of opf, xshift=-39.5pt] (shf) {Barrel Shifter};
  \node[halfmod=moegreen, above=27pt of opf, xshift=39.5pt] (add) {$3\times$ KS Adders};
  \coordinate (exe) at ($(shf.north)!0.5!(add.north)$);
  \coordinate (exes) at ($(shf.south)!0.5!(add.south)$);
  \node[module=mlared] (res) at ($(dec)+(0,4*46pt)$) {Flags $\cdot$ Result Mux};
  \node[module=mlared, above=27pt of res] (npc) {Next-PC Mux};
  \node[module=attnblue, above=27pt of npc] (rwb) {Register Writeback};
  ...
  \node[oplus, above=15pt of rwb] (p7) {};
  ...
  \draw[flow] (p3.north) -- ++(0,5pt) coordinate (fork);
  \draw[flow] (fork) -| (shf.south);
  \draw[flow] (fork) -| (add.south);
  \draw[flow] (shf.north) |- ($(p4.south)+(0,-5pt)$) -- (p4.south);
  \draw[wire] (add.north) |- ($(p4.south)+(0,-5pt)$);
  ...
  % residual skip wires (left lane)
  \coordinate (bin) at ($(dec.south)+(0,-24pt)$);
  \draw[flow, rounded corners=3pt] ($(bin)+(-4pt,0)$) -- ++(-92pt,0) |- ([yshift=-6pt]p1.west);
  \foreach \p/\q in {p1/p2, p2/p3, p3/p4, p4/p5, p5/p6, p6/p7}
    \draw[flow, rounded corners=3pt] (\p.west) -- ++(-88pt,0) |- (\q.west);

  % block indices
  \foreach \m/\l in {dec/B0--B1, pwb/B2, opf/B3, res/B12--B13, npc/B14, rwb/B15}
    \node[t8, anchor=west] at ($(\m.east)+(7pt,0)$) {\l};
  \node[t8, anchor=west] at ($(add.east)+(7pt,0)$) {B4--B11};

  % embedding chip + input
  \node[chip, below=24pt of dec] (emb) {Embed Machine State};
  \node[t8, below=4pt of emb, align=center]
    {34 tokens $\times$ 1187 dims:\ \ CTL $\cdot$ INSTR $\cdot$ x0\,\dots\,x31\\
  ...
  % ============================ RAM + recurrence ============================
  \node[ramchip] (ram) at ($(add.east)+(102pt,0)$) {RAM};
  \node[t8, below=3pt of ram] {the bus};
  \draw[busarr, rounded corners=3pt] ($(ram.south)+(-20pt,0)$) |- ($(emb.east)+(0,3pt)$);
  \node[t8, anchor=west, text=buscol] at ($(ram.south)+(-16pt,-30pt)$) {fetch RAM[PC]};
  \draw[busarr, rounded corners=3pt] ($(p7.east)+(0,0)$) -- ++(132pt,0) |- ($(ram.north)+(-12pt,14pt)$) -- ($(ram.north)+(-12pt,0)$);
  \node[t8, anchor=west, text=buscol, align=left] at ($(p7.east)+(140pt,0pt)$) {load $\cdot$ store $\cdot$ ecall};
  ...
  % main dashed frame
  \coordinate (frSW) at ($(dec.west)+(-104pt,-32pt)$);
  \coordinate (frNE) at ($(rwb.east)+(48pt,76pt)$);
