...
\begin{tikzpicture}[x=1pt, y=1pt, outer sep=0pt,
  ...
  \node[halfmod=moegreen, above=64pt of opf, xshift=-39.5pt] (shf) {Barrel Shifter};
  \node[halfmod=moegreen, above=64pt of opf, xshift=39.5pt] (add) {$3\times$ KS Adders};
  \coordinate (exe) at ($(shf.north)!0.5!(add.north)$);
  \coordinate (exes) at ($(shf.south)!0.5!(add.south)$);
  \node[module=mlared] (res) at ($(dec)+(0,4*67pt+16pt)$) {Flags $\cdot$ Result Mux};
  ...
  % residual skip wires: tap the trunk below each module, rejoin above it
  \coordinate (bin) at ($(emb.north)+(0,14pt)$);
  \draw[flow, rounded corners=3pt] (bin) -- ++(-92pt,0) |- (p1.west);
  \foreach \p/\q in {p1/p2, p2/p3, p3/p4, p4/p5, p5/p6, p6/p7}
    \draw[flow, rounded corners=3pt] ($(\p.north)+(0,8pt)$) -- ++(-92pt,0) |- (\q.west);
  ...
  \coordinate (fork) at ($(p3.north)+(0,18pt)$);
  \draw[wire] (p3.north) -- (fork);
  \draw[flow, rounded corners=3pt] (fork) -| (shf.south);
  \draw[flow, rounded corners=3pt] (fork) -| (add.south);
  \draw[wire, rounded corners=3pt] (shf.north) |- ($(p4.south)+(0,-8pt)$);
  \draw[wire, rounded corners=3pt] (add.north) |- ($(p4.south)+(0,-8pt)$);
  \draw[flow] ($(p4.south)+(0,-8pt)$) -- (p4.south);
