...
\begin{tikzpicture}[x=1pt, y=1pt, outer sep=0pt,
  ...
  \node[halfmod=moegreen, above=48pt of opf, xshift=-39.5pt] (shf) {Barrel Shifter};
  \node[halfmod=moegreen, above=48pt of opf, xshift=39.5pt] (add) {$3\times$ KS Adders};
  \coordinate (exe) at ($(shf.north)!0.5!(add.north)$);
  \coordinate (exes) at ($(shf.south)!0.5!(add.south)$);
  \node[module=mlared] (res) at ($(dec)+(0,4*67pt)$) {Flags $\cdot$ Result Mux};
  ...
  \coordinate (bin) at ($(dec.south)+(0,-18pt)$);
